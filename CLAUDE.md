...
- **Addition 1 — DONE (minimal scope).** `table_fraction@k` in `retrieval_metrics.py`; `retrieved_chunk_types` + `detected_institutions` in eval-runner output; sliced by query_type + probe.
- **Addition 3 — DONE.** 5 `[PROBE: table_only]` queries appended to ground truth (WEO LCOE, BoE TFSME WACI, CCC Progress buildings rollback, Seventh CB transport modal shift, WEO 2035 STEPS electricity). Reference answers grounded in actual DuckDB chunks before authoring. Ground truth total: 47 → 52.
- **Addition 2 — DEFERRED (v2).** Optional `expected_chunk_type` + `table_hit_rate` metric. Doesn't change v1 conclusion.
...
| ~~Metadata filtering (institution)~~ | **DONE Week 5 Step 2f.** Institution detector + Chroma/BM25 filter promoted to v1. On 47-query A/B: cross-doc Completeness +1.0, all retrieval metrics up ~+0.03, refusal_appr +0.26 (including fixing the SMR fabrication), latency +18ms. See `docs/week5_failure_analysis.md § 2f`. | Done |
| Metadata filtering (jurisdiction, date) | Only `institution` shipped in v1. Extending to jurisdiction / publication_date filters is a v2 candidate — same mechanism, different metadata columns. | Medium |
| Per-source retrieval union | Metadata filtering narrows the pool but doesn't guarantee per-source top-5 coverage (a named institution's chunks can still lose to a co-named institution's chunks inside the filtered pool). Fix: retrieve top-N per named institution independently, then union before RRF. Guarantees coverage. See `docs/week5_failure_analysis.md § 2f "Known limitations"`. | Medium |
