...
def run_verifier(state: dict) -> dict:
    ...
    # Build set of all chunk_ids that were actually retrieved
    known_ids: set[str] = {
        chunk.get("chunk_id")
    ...
    for raw_claim in claims:
        ...
        valid_eids = [e for e in claim.evidence_ids if e in known_ids]

        if not valid_eids:
            logger.info(
                "Verifier: dropped claim %r — none of %s found in retrievals",
                claim.id,
                claim.evidence_ids,
            )
            continue
