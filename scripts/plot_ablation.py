...
def _grouped_bars(ax, categories, series, colors, labels, ylim=None, ylabel=None, fmt="{:.2f}", annotate=True):
...
def chart_01_aggregate(data, ax=None, standalone=True):
    ...
    series = [[get_overall(data, cfg, m) for m in metrics] for cfg in CONFIG_ORDER]
    colors = [CONFIG_COLORS[c] for c in CONFIG_ORDER]

    # Latency-annotated config labels
    labels = [f"{CONFIG_LABELS[c]} · {get_latency(data, c):.0f}ms" for c in CONFIG_ORDER]
    ...
    _grouped_bars(ax, metric_labels, series, colors, labels, ylim=(0, 5.4), ylabel="Judge score (1–5)")

    ax.set_title(
        "Aggregate judge scores across 47 queries",
        loc="left",
        fontweight="bold",
        fontsize=12,
        pad=12,
...
def chart_02_by_query_type(data, ax=None, standalone=True):
    ...
    series = [[get_type_metric(data, cfg, qt, "correctness") for qt in qtypes] for cfg in CONFIG_ORDER]
    colors = [CONFIG_COLORS[c] for c in CONFIG_ORDER]
    ...
    _grouped_bars(ax, qtype_labels, series, colors, labels, ylim=(0, 5.4), ylabel="Correctness (1–5)")

    ax.set_title(
        "Correctness by query type — where the aggregate tie breaks",
        loc="left",
        fontweight="bold",
        fontsize=12,
        pad=12,
...
def chart_03_retrieval(data, ax=None, standalone=True):
    ...
    ax.set_title(
        "Retrieval metrics — chunk-level ranking quality",
        loc="left",
        fontweight="bold",
        fontsize=12,
        pad=12,
...
def chart_04_negatives(data, ax=None, standalone=True):
    ...
    ax.set_title(
        "Out-of-corpus negatives — did the system refuse when it should?",
        loc="left",
        fontweight="bold",
        fontsize=12,
        pad=12,
    ...
    ax.text(len(labels) - 0.5, 1.01, "target = 100%", ha="right", va="bottom", fontsize=8, color="#999")

    if standalone:
        fig.text(0.5, -0.02, FOOTNOTE + "  · n=9 negatives", ha="center", fontsize=7.5, color="#666")
        fig.tight_layout()
...
def chart_composite(data):
    ...
    gs = fig.add_gridspec(2, 2, hspace=0.35, wspace=0.18, left=0.06, right=0.98, top=0.90, bottom=0.06)
    ...
    fig.suptitle(
        "CPIE — 4-config retrieval ablation",
        x=0.06,
        y=0.965,
        ha="left",
        fontsize=18,
